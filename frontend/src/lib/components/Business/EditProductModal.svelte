<script>
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await ProductApi.updateProduct(product.productId, productData);
      
      toastStore.add('상품이 수정되었습니다.', 'success');
      handleClose();
...
<Modal bind:open title="상품 수정" size="xl" on:close={handleClose}>
      ...
      {#if product}
        <div class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded">
          <p class="text-sm text-blue-800">
            <strong>현재 재고:</strong> {product.stockQuantity}개
          </p>
          <p class="text-xs text-blue-600 mt-1">
            재고 수량 변경은 별도의 재고 조정 기능을 사용하세요
          </p>
        </div>
      {/if}
    </div>
