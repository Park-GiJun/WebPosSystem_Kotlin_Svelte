...
BEGIN
    ...
    INSERT INTO permissions (permission_id, permission_type, target_type, menu_id, target_id, granted_by, created_at, is_active)
    SELECT 
        UUID(),
        'ADMIN',
        'USER',
    ...
    INSERT INTO permissions (permission_id, permission_type, target_type, menu_id, target_id, granted_by, created_at, is_active)
    VALUES (UUID(), 'WRITE', 'USER', 'ADMIN_USERS', p_admin_user_id, 'system', current_time, 1);
    
    -- 4. 본사에게 사용자 생성 권한 부여 (조직 권한)
    INSERT INTO permissions (permission_id, permission_type, target_type, menu_id, target_id, granted_by, created_at, is_active)
    VALUES (UUID(), 'WRITE', 'ORGANIZATION', 'ADMIN_USERS', p_hq_id, 'system', current_time, 1);
END$$
...
BEGIN
    ...
    INSERT INTO permissions (permission_id, permission_type, target_type, menu_id, target_id, granted_by, created_at, is_active)
    VALUES (UUID(), 'WRITE', 'ORGANIZATION', 'ADMIN_USERS', p_store_id, 'system', current_time, 1);
    ...
    IF NOT p_is_chain THEN
        ...
        INSERT INTO permissions (permission_id, permission_type, target_type, menu_id, target_id, granted_by, created_at, is_active)
        VALUES 
            (UUID(), 'READ', 'USER', 'BUSINESS_STORES', p_admin_user_id, 'system', current_time, 1),
            (UUID(), 'READ', 'USER', 'BUSINESS_REPORTS', p_admin_user_id, 'system', current_time, 1);
    END IF;
END$$
